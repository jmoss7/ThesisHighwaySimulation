...
from srunner.tests.carla_mocks.agents.navigation.behavior_types import Cautious
from srunner.tests.carla_mocks.agents.navigation.global_route_planner import GlobalRoutePlanner

sys.path.insert(0, 'C:\\Users\\jpmoc\\Documents\\CSclasses\\masters\\thesis\\carla\\CARLA_0.9.15\\WindowsNoEditor\\PythonAPI\\carla\\agents')
import carla
...
class BasicAgentBehavior(AtomicBehavior):
    ...
    def __init__(self, actor, target_location=None, plan=None, target_speed=20, opt_dict=None, name="BasicAgentBehavior"):
        ...
        #     print("VEHICLE {} IS PROCEEDING TO THE EXIT".format(actor.id))
        # else:
        #     print("VEHICLE {} IS SUPPOSED TO DRIVE STRAIGHT".format(actor.id))
    ...
    def initialise(self):
        ...
        self._agent._ignore_vehicles = False
        self._agent._use_bbs_detection = True
        self._agent._base_vehicle_threshold = 5
        # self._agent.behavior = Cautious()
        if self._plan:
            ...
            init_wp = CarlaDataProvider.get_map().get_waypoint(self._actor.get_location())
            end_wp = self._map.get_waypoint(self._target_location)
            self._plan = self._agent.trace_route(init_wp, end_wp)
            self._agent.set_global_plan(self._plan)
            # self._agent.set_destination(end_wp)
...
class GetOverAgentBehavior(AtomicBehavior):
    """
    This class contains an atomic behavior, which uses the
    basic_agent from CARLA to control the actor until
    reaching a target location.
    ...
    def __init__(self, actor, target_lane_id, target_location=None, plan=None, target_speed=20, opt_dict=None, name="GetOverAgentBehavior"):
        """
        Setup actor and maximum steer value
        """
        super(GetOverAgentBehavior, self).__init__(name, actor)
        self._map = CarlaDataProvider.get_map()
        self._target_location = target_location
    ...
    def initialise(self):
        ...
        self._agent._ignore_vehicles = False
        self._agent._use_bbs_detection = True
        self._agent._base_vehicle_threshold = 10
        # self._agent.behavior = Cautious()
        if self._plan:
            self._agent.set_global_plan(self._plan)
        elif self._target_location:
    ...
    def update(self):
        ...
        # self._plan = self._agent.trace_route(init_wp, end_wp)
        # self._agent.set_global_plan(self._plan)
        current_waypoint = self._agent._vehicle.get_world().get_map().get_waypoint(self._agent._vehicle.get_location())
        if self._agent._vehicle.get_world().get_map().get_waypoint(self._agent._vehicle.get_location()).lane_id == self._target_lane_id:
            new_status = py_trees.common.Status.SUCCESS
        #self._agent.lane_change('right', 3, 3, 3)
        self._agent.set_destination(current_waypoint.get_right_lane(), current_waypoint)
        self._control = self._agent.run_step()
        self._actor.apply_control(self._control)
    ...
    def terminate(self, new_status):
        ...
        self._control.brake = 0.0
        self._actor.apply_control(self._control)
        super(GetOverAgentBehavior, self).terminate(new_status)
...
class LaneChange(WaypointFollower):
    ...
    def update(self):
        ...
        print("VEHICLE {} IS TRYING TO LANECHANGE!".format(self._actor.id))
        if not self._plan:
            print("{} couldn't perform the expected lane change".format(self._actor))
            return py_trees.common.Status.RUNNING
